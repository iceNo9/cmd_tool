...
class Logger:
    ...
    def __init__(
        ...
        if use_file:
            self._add_file_handler(
                file_level, max_bytes, backup_count, use_timed_rotation
            )
    ...
    def _add_file_handler(
        self, level: int, max_bytes: int, backup_count: int, use_timed_rotation: bool
    ):
        """添加文件日志处理器"""
        # 创建日志目录
        ...
        log_file = os.path.join(
            self.log_dir,
            f'{self.name}_{datetime.now(tz=UTC).strftime("%Y%m%d")}.log',
        )
...
if __name__ == "__main__":
    ...
    # 测试配置更新
    print("\n4. 测试配置更新：")
    configure_logger({"name": "new_app", "log_dir": "logs/new", "fmt_type": "simple"})
    info("使用新配置的日志")
