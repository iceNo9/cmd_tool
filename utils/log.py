...
if __name__ == "__main__":
    ...
    # 测试按模块分文件
    print("\n3. 测试模块日志：")
    db_logger = get_logger("database", log_dir="logs", fmt_type="detailed")
    db_logger.info("数据库连接成功")
    db_logger.error("数据库查询失败")

    parser_logger = get_logger("manifest_parser", log_dir="logs")
    parser_logger.info("开始解析 manifest")

    loader_logger = get_logger("plugin_loader", log_dir="logs")
    loader_logger.info("开始扫描插件")
